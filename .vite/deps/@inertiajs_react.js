...
function mergeDataIntoQueryString(method2, href, data, qsArrayFormat = "brackets") {
  const hasDataForQueryString = method2 === "get" && !isFormData2(data) && Object.keys(data).length > 0;
...
var EventHandler = class {
  ...
  init() {
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("scroll", debounce3(Scroll.onWindowScroll.bind(Scroll), 100), true);
    }
    if (typeof document !== "undefined") {
      document.addEventListener("scroll", debounce3(Scroll.onScroll.bind(Scroll), 100), true);
    }
  }
...
var objectsAreEqual = (obj1, obj2, excludeKeys) => {
  ...
  for (const key in obj1) {
    ...
    if (!compareValues3(obj1[key], obj2[key])) {
      return false;
    }
  }
  return true;
};
var compareValues3 = (value1, value2) => {
...
var Router = class {
  ...
  prefetch(href, options = {}, prefetchOptions = {}) {
    const method2 = options.method ?? (isUrlMethodPair(href) ? href.method : "get");
    if (method2 !== "get") {
...
function undotKey(key) {
  ...
  const transformSegment = (segment) => {
    ...
    return segment.split(".").reduce((result2, part, index) => index === 0 ? part : `${result2}[${part}]`);
  };
...
function resetFormFields(formElement, defaults3, fieldNames) {
  if (!formElement) {
    return;
  }
  if (!fieldNames || fieldNames.length === 0) {
    ...
    fieldNames = [.../* @__PURE__ */ new Set([...defaults3.keys(), ...formData.keys(), ...formElementNames])];
  }
  let hasChanged = false;
  fieldNames.forEach((fieldName) => {
    const elements = formElement.elements.namedItem(fieldName);
    if (elements) {
      if (resetFieldElements(elements, defaults3.getAll(fieldName))) {
        hasChanged = true;
      }
    }
  });
...
var Renderer = {
  ...
  update: debounce3(function(elements) {
...
var increaseByRandom = (amount) => {
  ...
  return set2(clamp3(n + amount, 0, 0.994));
};
...
function useForm(rememberKeyOrInitialValues, maybeInitialValues) {
  ...
  const [defaults3, setDefaults] = (0, import_react8.useState)(
  ...
  const [data, setData] = rememberKey ? useRemember(defaults3, `${rememberKey}:data`) : (0, import_react8.useState)(defaults3);
  ...
  const [progress3, setProgress] = (0, import_react8.useState)(null);
  ...
  const transform2 = (0, import_react8.useRef)((data2) => data2);
  const isDirty = (0, import_react8.useMemo)(() => !isEqual(data, defaults3), [data, defaults3]);
    ...
    (fieldOrFields, maybeValue) => {
      if (typeof fieldOrFields === "undefined") {
        ...
        setDefaults((defaults22) => {
          return typeof fieldOrFields === "string" ? set(cloneDeep(defaults22), fieldOrFields, maybeValue) : Object.assign(cloneDeep(defaults22), fieldOrFields);
        });
      }
    },
    [data, setDefaults]
  ...
  const reset = (0, import_react8.useCallback)(
    (...fields) => {
      if (fields.length === 0) {
        ...
        setData(
          (data2) => fields.filter((key) => has(defaults3, key)).reduce(
            (carry, key) => {
              return set(carry, key, get(defaults3, key));
            },
            { ...data2 }
          )
        );
...
var noop4 = () => void 0;
var Form = (0, import_react7.forwardRef)(
  ({
    action = "",
    method: method2 = "get",
    headers = {},
    ...
    errorBag = null,
    showProgress = true,
    transform: transform2 = (data) => data,
    options = {},
    onStart = noop4,
    onProgress = noop4,
    onFinish = noop4,
    ...
    const form = useForm({});
    const formElement = (0, import_react7.useRef)(null);
    const resolvedMethod = (0, import_react7.useMemo)(() => {
      return typeof action === "object" ? action.method : method2.toLowerCase();
    }, [action, method2]);
    const [isDirty, setIsDirty] = (0, import_react7.useState)(false);
    const defaultData = (0, import_react7.useRef)(new FormData());
    ...
    const submit = () => {
      ...
      form.transform(() => transform2(_data));
    ...
    (0, import_react7.useImperativeHandle)(ref, exposed, [form, isDirty, submit]);
